(function () {
  ...
  function bindControls() {
    ...
    if (els.siReset) els.siReset.addEventListener("click", function () {
      ...
      STATE.surveyVictimizationPeriod = "all";
      ...
      STATE.surveyReportingPeriod = "all";
  ...
  function populateSurveyFilters() {
    if (!STATE.surveyLoaded) {
      [
        ...
        populateControlGroup(name, [{ value: "all", label: ui("Caricamento dati", "Loading data") }], "all");
        setControlDisabled(name, true);
      });
      return;
    }
  ...
  function populateSurveyTopicFilters(topic, keys) {
    ...
    populateControlGroup(keys.indicator, indicatorOptions, STATE[keys.indicator]);

    var scoped = filterSurveyRows(rows, keys, { skip: "dimension" });
    var dimensionOptions = [{ value: "all", label: ui("Tutti i dettagli", "All details") }].concat(
      unique(scoped.map(function (row) { return row.dimension_type; })).filter(Boolean).sort(sortItalian).map(function (value) {
        return { value: value, label: surveyDimensionLabel(value) };
      })
    );
    keepValidSelection(keys.dimension, dimensionOptions, "all");
    populateControlGroup(keys.dimension, dimensionOptions, STATE[keys.dimension]);

    if (keys.crime) {
      scoped = filterSurveyRows(rows, keys, { skip: "crime" });
      ...
      populateControlGroup(keys.crime, crimeOptions, STATE[keys.crime]);
    }

    scoped = filterSurveyRows(rows, keys, { skip: "response" });
    var responseOptions = [{ value: "all", label: ui("Tutte le voci", "All items") }].concat(
      unique(scoped.map(surveyResponseValue)).filter(Boolean).sort(sortItalian).map(optionFromValue)
    );
    keepValidSelection(keys.response, responseOptions, "all");
    populateControlGroup(keys.response, responseOptions, STATE[keys.response]);

    if (keys.unit) {
      scoped = filterSurveyRows(rows, keys, { skip: "unit" });
      var unitOptions = [{ value: "all", label: ui("Tutte le unità", "All units") }].concat(
        unique(scoped.map(function (row) { return row.unit; })).filter(Boolean).sort(sortItalian).map(optionFromValue)
      );
      keepValidSelection(keys.unit, unitOptions, "all");
      populateControlGroup(keys.unit, unitOptions, STATE[keys.unit]);
    }

    scoped = filterSurveyRows(rows, keys, { skip: "period" });
    var periodOptions = [{ value: "all", label: ui("Tutti i periodi", "All periods") }].concat(
      unique(scoped.map(function (row) { return row.period; })).filter(Boolean).sort(sortItalian).map(optionFromValue)
    );
    keepValidSelection(keys.period, periodOptions, "all");
    populateControlGroup(keys.period, periodOptions, STATE[keys.period]);

    Object.keys(keys).forEach(function (key) {
      setControlDisabled(keys[key], !enabled);
    });
  }
  ...
  function filterSurveyRows(rows, keys, options) {
    options = options || {};
    return (rows || []).filter(function (row) {
      if (options.skip !== "indicator" && STATE[keys.indicator] && STATE[keys.indicator] !== "all" && row.indicator !== STATE[keys.indicator]) return false;
      if (options.skip !== "dimension" && STATE[keys.dimension] && STATE[keys.dimension] !== "all" && row.dimension_type !== STATE[keys.dimension]) return false;
      if (keys.crime && options.skip !== "crime" && STATE[keys.crime] && STATE[keys.crime] !== "all" && row.crime_name !== STATE[keys.crime]) return false;
      if (keys.unit && options.skip !== "unit" && STATE[keys.unit] && STATE[keys.unit] !== "all" && row.unit !== STATE[keys.unit]) return false;
      if (options.skip !== "response" && STATE[keys.response] && STATE[keys.response] !== "all" && surveyResponseValue(row) !== STATE[keys.response]) return false;
      if (options.skip !== "period" && STATE[keys.period] && STATE[keys.period] !== "all" && row.period !== STATE[keys.period]) return false;
      return true;
    });
  }
  ...
  function surveyUnitLabel(rows) {
    ...
    if (units.length === 1) return tr(units[0]);
    return ui("Valore percentuale", "Percentage value");
  }

  function surveyChartTag(topic, rows) {
    var keys = surveyControlKeys(topic);
    var period = STATE[keys.period] && STATE[keys.period] !== "all" ? STATE[keys.period] : ui("tutti i periodi", "all periods");
  ...
  function surveyNarrativeTitle(topic) {
    ...
    if (STATE[keys.response] && STATE[keys.response] !== "all") parts.push(STATE[keys.response]);
  ...
  function surveyNarrativeNote(topic) {
    ...
    if (topic === "reporting_propensity") {
      return ui("Propensione alla denuncia da indagine campionaria: ", "Reporting propensity from sample survey: ") + unit + ui(". Le tavole aggregate mostrano reato, ripartizione geografica, motivi e caratteristiche dell'episodio quando disponibili.", ". Aggregate tables show offence, geographic area, reasons and episode characteristics where available.");
    }
    if (topic === "victimization") {
      return ui("Vittimizzazione dichiarata da indagine campionaria: ", "Reported victimization from sample survey: ") + unit + ui(". Include anche fenomeni non denunciati e non va sommata ai delitti registrati.", ". It also includes unreported events and should not be added to registered crimes.");
    }
    return ui("Percezione della sicurezza da indagine campionaria: ", "Perceived safety from sample survey: ") + unit + ui(". Misura opinioni, rischio percepito e comportamenti dichiarati, non denunce registrate.", ". It measures opinions, perceived risk and stated behaviours, not registered reports.");
  }
