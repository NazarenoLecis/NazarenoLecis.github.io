(function () {
  ...
  var MEASURE_OPTIONS = [
    ...
    { value: "rate_1000", label: "Tassi per 1.000" },
  ...
  var PEOPLE_MEASURE_OPTIONS = [
    ...
    { value: "rate", label: "Tasso per 100.000 abitanti, se disponibile" },
    { value: "rate_1000", label: "Tasso per 1.000 abitanti, se disponibile" }
  ];
  var COUNTER_OPTIONS = [
  ...
  function renderPeopleNotice() {
    ...
    els.siPeopleNotice.innerHTML = "<strong>Copertura persone:</strong> " + escapeHtml(scope) + ". I grafici separano sempre autori e vittime. Il filtro Misura permette conteggi e quote; i tassi per cittadinanza, eta, sesso e territorio vengono mostrati solo quando il payload contiene denominatori coerenti.";
  }
  ...
  function renderCoverage() {
    els.siCoverage.innerHTML = [
      ...
      coverageItem("Autori e vittime", "Sono persone registrate nelle fonti disponibili; quote e tassi dipendono dalla misura scelta."),
  ...
  function renderHomicideTrendChart() {
    ...
    if (STATE.measure === "rate" || STATE.measure === "rate_1000") {
      return emptyChart("siHomicideTrendChart", "I tassi per abitante non sono disponibili nel payload corrente per questa vista: usa valori assoluti, indice o variazione annua.");
    }
  ...
  function renderHomicideBreakdownView() {
    if (STATE.measure === "rate" || STATE.measure === "rate_1000" || STATE.measure === "index" || STATE.measure === "moving_average") {
  ...
  function peopleMetricValueForGroup(group, total) {
    ...
    if (STATE.peopleMeasure === "rate" || STATE.peopleMeasure === "rate_1000") {
      var rows = group.rows || [];
      if (!rows.length || rows.some(function (row) { return !isFiniteNumber(row.value_rate_per_100k); })) return null;
      var rate100k = sum(rows, function (row) { return row.value_rate_per_100k; });
      return STATE.peopleMeasure === "rate_1000" ? rate100k / 100 : rate100k;
    }
    return group.value;
  }
  ...
  function peopleRateUnavailableMessage() {
    if (STATE.peopleMeasure === "rate" || STATE.peopleMeasure === "rate_1000") {
      return "Il tasso richiesto non e disponibile nel payload corrente: servono denominatori coerenti per cittadinanza, eta, sesso e territorio. Usa valori assoluti o quota %.";
    }
  ...
  function homicideTerritoryMetric(row) {
    if (STATE.measure === "rate" || STATE.measure === "rate_1000") return null;
  ...
  function valueForMeasureBase(row) {
    ...
    if (STATE.measure === "rate_1000") return isFiniteNumber(row.value_rate_per_100k) ? row.value_rate_per_100k / 100 : null;
    return row.value;
  }
