(function () {
  ...
  var PAYLOAD_GLOBALS = {
    ...
    securitySpendingPublicTotal: "SICUREZZA_SECURITY_SPENDING_PUBLIC_TOTAL",
    securitySpendingPublicDetail: "SICUREZZA_SECURITY_SPENDING_PUBLIC_DETAIL",
    securitySpendingPrivate: "SICUREZZA_SECURITY_SPENDING_PRIVATE"
  };
  var DATA_BASES = [
  ...
  var SECURITY_SPENDING_PAYLOADS = {
    publicTotal: { file: "dashboard_security_spending_public_total.js", global: PAYLOAD_GLOBALS.securitySpendingPublicTotal },
    publicDetail: { file: "dashboard_security_spending_public_detail.js", global: PAYLOAD_GLOBALS.securitySpendingPublicDetail },
    privateMarket: { file: "dashboard_security_spending_private.js", global: PAYLOAD_GLOBALS.securitySpendingPrivate }
  };
  var SECURITY_DOMAIN_OPTIONS = [
    { value: "public_security_spending", label: "Spesa pubblica per sicurezza" },
    { value: "private_security_market", label: "Mercato privato della sicurezza" }
  ];
  var SECURITY_PUBLIC_CLASSIFICATIONS = [
    { value: "GF03", label: "Totale ordine pubblico e sicurezza" },
    { value: "GF0301", label: "Servizi di polizia" },
    { value: "GF0302", label: "Servizi antincendio" },
    { value: "GF0303", label: "Tribunali" },
    { value: "GF0304", label: "Istituti penitenziari" },
    { value: "GF0305", label: "Ricerca e sviluppo sicurezza" },
    { value: "GF0306", label: "Altra spesa sicurezza n.c.a." }
  ];
  var SECURITY_PRIVATE_CLASSIFICATIONS = [
    { value: "N80", label: "Totale investigazione e sicurezza" },
    { value: "N801", label: "Vigilanza privata" },
    { value: "N802", label: "Sistemi di sicurezza" },
    { value: "N803", label: "Investigazioni" }
  ];
  var SECURITY_PUBLIC_MEASURE_OPTIONS = [
    { value: "PC_GDP", label: "% del PIL" },
    { value: "PC_TOT", label: "% della spesa pubblica" },
    { value: "EUR_HAB_COMPUTED", label: "Euro per abitante" },
    { value: "MIO_EUR", label: "Miliardi di euro" }
  ];
  var SECURITY_PUBLIC_DEFAULT_MEASURE = "PC_GDP";
  var SECURITY_PRIVATE_DEFAULT_MEASURE = "NETTUR_MEUR||MIO_EUR";
  var SECURITY_TRANSFORM_OPTIONS = [
  ...
  var TEXT_EN = {
    ...
    "Delitti denunciati o accertati dalle Forze di polizia e trasmessi all'autorità giudiziaria. Sono la fonte più granulare per territorio, anno e tipo di reato, ma riflettono anche propensione e possibilità concreta di denunciare.": "Crimes reported to or detected by police forces and forwarded to the judicial authority. This is the most granular source by territory, year and offence type, but it also reflects people's propensity and practical ability to report.",
    ...
    "Spesa e mercato": "Spending and market",
    "Risorse pubbliche e sicurezza privata": "Public resources and private security",
    "Questa sezione affianca alle denunce il contesto macro: spesa pubblica per ordine pubblico e sicurezza e proxy del mercato privato della sicurezza.": "This section adds the macro context to reported-crime data: public spending on public order and safety and a proxy for the private security market.",
    "la spesa pubblica usa COFOG GF03, distinta dalla difesa COFOG GF02. La vista si apre sulla spesa pubblica in percentuale del PIL; le altre unità restano selezionabili. La sicurezza privata usa Eurostat NACE N80 come proxy di mercato: non misura tutta la spesa privata effettiva di famiglie e imprese.": "public spending uses COFOG GF03, separate from defence COFOG GF02. The view opens on public spending as a percentage of GDP; the other units remain selectable. Private security uses Eurostat NACE N80 as a market proxy: it does not measure all actual private spending by households and firms.",
    "Serie storica della spesa": "Spending time series",
    "Scomposizione micro": "Micro breakdown",
    ...
    "serie annuale macro, con valori normalizzabili in indice o variazione annua. Fonte: Eurostat. Elaborazione di Nazareno Lecis.": "annual macro series, with values that can be normalized as an index or annual change. Source: Eurostat. Processing by Nazareno Lecis.",
    "sottovoci COFOG GF03 per la spesa pubblica e sottosettori NACE N80 per la sicurezza privata. Fonte: Eurostat. Elaborazione di Nazareno Lecis.": "COFOG GF03 sub-items for public spending and NACE N80 sub-sectors for private security. Source: Eurostat. Processing by Nazareno Lecis.",
    "confronto fra paesi europei disponibili nella fonte; l'Italia è evidenziata. Fonte: Eurostat. Elaborazione di Nazareno Lecis.": "comparison across European countries available in the source; Italy is highlighted. Source: Eurostat. Processing by Nazareno Lecis.",
    ...
    "Nota su denunce e accesso alla denuncia:": "Note on reports and access to reporting:",
    " i valori sui delitti denunciati dipendono anche dalla propensione e dalla possibilità concreta di denunciare. Difficoltà burocratiche, linguistiche o di accesso ai servizi possono ridurre le denunce; campagne di sensibilizzazione, canali più semplici o cambiamenti normativi possono aumentarle senza implicare necessariamente più reati commessi.": " reported-crime values are also affected by people's propensity and practical ability to report. Bureaucratic, language or service-access barriers can reduce reports; awareness campaigns, simpler channels or legal changes can increase them without necessarily implying more offences committed.",
    ...
    "Spesa pubblica per sicurezza": "Public security spending",
    "Mercato privato della sicurezza": "Private security market",
    "Totale ordine pubblico e sicurezza": "Total public order and safety",
    "Servizi di polizia": "Police services",
    "Servizi antincendio": "Fire-protection services",
    "Tribunali": "Law courts",
    "Istituti penitenziari": "Prisons",
    "Ricerca e sviluppo sicurezza": "Security R&D",
    "Altra spesa sicurezza n.c.a.": "Other public-order and safety spending n.e.c.",
    "Totale investigazione e sicurezza": "Total security and investigation",
    "Vigilanza privata": "Private security activities",
    "Sistemi di sicurezza": "Security systems service activities",
    "Investigazioni": "Investigation activities",
    "Miliardi di euro": "Billion euro",
    "Milioni di euro": "Million euro",
    "Euro per abitante": "Euro per inhabitant",
    "% del PIL": "% of GDP",
    "% della spesa pubblica": "% of public spending",
    "Valore": "Value",
    "Fatturato netto - milioni di euro": "Net turnover - million euro",
    "Fatturato netto - euro per abitante": "Net turnover - euro per inhabitant",
    "Fatturato netto del settore sicurezza privata": "Net turnover of the private security sector",
    "Valore aggiunto del settore sicurezza privata": "Value added of the private security sector",
    "Persone occupate nel settore sicurezza privata": "Persons employed in the private security sector",
    "Imprese del settore sicurezza privata": "Enterprises in the private security sector",
    "Retribuzioni e salari del settore sicurezza privata": "Wages and salaries in the private security sector",
    "Investimenti lordi in beni materiali del settore sicurezza privata": "Gross investment in tangible goods in the private security sector",
    "milioni di euro": "million euro",
    "euro per abitante": "euro per inhabitant",
    "numero": "number",
  ...
  var CHART_IDS = [
    ...
    "siSecuritySpendingTrendChart", "siSecuritySpendingBreakdownChart", "siSecuritySpendingRankingChart"
  ];
  ...
  var CHART_FILTER_KEYS = [
    ...
    "homicideSex", "homicideAgeGroup", "securityDomain", "securityGeo", "securityCompareGeo",
    "securityClassification", "securityMeasure", "securityTransform", "securityYear", "search"
  ];
  ...
  var STATE = {
    ...
    securityDomain: "public_security_spending",
    securityGeo: "IT",
    securityCompareGeo: "EU27_2020",
    securityClassification: "GF03",
    securityMeasure: SECURITY_PUBLIC_DEFAULT_MEASURE,
    securityTransform: "value",
    securityYear: null,
    search: "",
    populationLoaded: false,
    peopleLoaded: false,
    surveyLoaded: false,
    securitySpendingLoaded: {},
    securitySpendingLoading: {},
  ...
  function init() {
    [
      ...
      "siSecuritySpendingTrendTag", "siSecuritySpendingBreakdownTag", "siSecuritySpendingRankingTag"
    ].forEach(function (id) {
  ...
  function bindControls() {
    ...
    if (els.siReset) els.siReset.addEventListener("click", function () {
      ...
      STATE.securityDomain = "public_security_spending";
      STATE.securityGeo = "IT";
      STATE.securityCompareGeo = "EU27_2020";
      STATE.securityClassification = "GF03";
      STATE.securityMeasure = SECURITY_PUBLIC_DEFAULT_MEASURE;
      STATE.securityTransform = "value";
      STATE.securityYear = null;
      STATE.search = "";
      STATE.chartStates = {};
  ...
  function applyControlValue(name, value) {
    ...
    if (name === "securityDomain") {
      STATE.securityDomain = value;
      STATE.securityClassification = value === "private_security_market" ? "N80" : "GF03";
      STATE.securityMeasure = value === "private_security_market" ? SECURITY_PRIVATE_DEFAULT_MEASURE : SECURITY_PUBLIC_DEFAULT_MEASURE;
      STATE.securityTransform = "value";
      STATE.securityYear = null;
    }
    if (name === "securityGeo") STATE.securityGeo = value;
    if (name === "securityCompareGeo") STATE.securityCompareGeo = value;
    if (name === "securityClassification") STATE.securityClassification = value;
    if (name === "securityMeasure") STATE.securityMeasure = value;
    if (name === "securityTransform") STATE.securityTransform = value;
    if (name === "securityYear") STATE.securityYear = numberOrNull(value);
  }
  ...
  function loadSecuritySpendingPayload(key) {
    if (STATE.securitySpendingLoaded[key]) return Promise.resolve(true);
    if (STATE.securitySpendingLoading[key]) return STATE.securitySpendingLoading[key];
    var payload = SECURITY_SPENDING_PAYLOADS[key];
    if (!payload) return Promise.resolve(false);
    STATE.securitySpendingLoading[key] = loadJavascriptPayload(payload.file, payload.global).then(function (data) {
      var rows = decodeColumnPayload(data, data && data.indicator_group || "security_spending");
      STATE.securitySpendingRecords = mergeSecuritySpendingRows(STATE.securitySpendingRecords, rows);
      STATE.securitySpendingLoaded[key] = true;
      delete STATE.securitySpendingLoading[key];
      return true;
    }).catch(function () {
      STATE.securitySpendingLoaded[key] = false;
      delete STATE.securitySpendingLoading[key];
      return false;
    });
    return STATE.securitySpendingLoading[key];
  }

  function mergeSecuritySpendingRows(existing, incoming) {
    var seen = {};
    (existing || []).forEach(function (row) {
      seen[securitySpendingRowKey(row)] = row;
    });
  ...
  function securitySpendingRowKey(row) {
    return [
      row.macro_domain || row.indicator_group || "",
      row.geo_code || "",
      row.classification_code || row.cofog_code || "",
      row.measure_code || row.private_indicator_code || "",
      row.unit_code || "",
      row.year || ""
    ].join("||");
  }
  ...
  function populateSecuritySpendingFilters(chartId) {
    populateControlGroup("securityDomain", SECURITY_DOMAIN_OPTIONS, STATE.securityDomain, chartId);

    var classificationOptions = STATE.securityDomain === "private_security_market"
      ? SECURITY_PRIVATE_CLASSIFICATIONS
      : SECURITY_PUBLIC_CLASSIFICATIONS;
    keepValidSelection(
      "securityClassification",
      classificationOptions,
      STATE.securityDomain === "private_security_market" ? "N80" : "GF03"
    );
    populateControlGroup("securityClassification", classificationOptions, STATE.securityClassification, chartId);

    var measureOptions = securityMeasureOptions();
    keepValidSelection("securityMeasure", measureOptions, measureOptions[0] ? measureOptions[0].value : SECURITY_PUBLIC_DEFAULT_MEASURE);
    populateControlGroup("securityMeasure", measureOptions, STATE.securityMeasure, chartId);
    populateControlGroup("securityTransform", SECURITY_TRANSFORM_OPTIONS, STATE.securityTransform, chartId);

    var geoOptions = securityGeoOptions();
    keepValidSelection("securityGeo", geoOptions, geoOptions.some(function (option) { return option.value === "IT"; }) ? "IT" : (geoOptions[0] ? geoOptions[0].value : "IT"));
    populateControlGroup("securityGeo", geoOptions, STATE.securityGeo, chartId);
    var compareOptions = [{ value: "none", label: ui("Nessun confronto", "No comparison") }].concat(
      geoOptions.filter(function (option) { return option.value !== STATE.securityGeo; })
    );
    keepValidSelection("securityCompareGeo", compareOptions, compareOptions.some(function (option) { return option.value === "EU27_2020"; }) ? "EU27_2020" : "none");
    populateControlGroup("securityCompareGeo", compareOptions, STATE.securityCompareGeo, chartId);

    var yearOptions = securityYearOptions(chartId);
    if (!STATE.securityYear || !yearOptions.some(function (option) { return Number(option.value) === Number(STATE.securityYear); })) {
      STATE.securityYear = yearOptions.length ? numberOrNull(yearOptions[0].value) : null;
    }
    populateControlGroup("securityYear", yearOptions, STATE.securityYear, chartId);
  }

  function securityMeasureOptions() {
    if (STATE.securityDomain !== "private_security_market") return SECURITY_PUBLIC_MEASURE_OPTIONS;
    var rows = securitySpendingRows().filter(function (row) {
      return row.macro_domain === "private_security_market" && row.classification_code === STATE.securityClassification;
    });
    var options = [];
    rows.forEach(function (row) {
      var key = securityPrivateMeasureKey(row);
      if (!key || options.some(function (option) { return option.value === key; })) return;
      options.push({
        value: key,
        label: securityPrivateMeasureLabel(row)
      });
    });
    if (!options.length) {
      options = [
        { value: "NETTUR_MEUR||MIO_EUR", label: "Fatturato netto - milioni di euro" },
        { value: "NETTUR_MEUR||EUR_HAB_COMPUTED", label: "Fatturato netto - euro per abitante" }
      ];
    }
    return options.sort(function (a, b) { return sortItalian(a.label, b.label); });
  }

  function securityGeoOptions() {
    var rows = securitySpendingRows().filter(function (row) {
      return row.macro_domain === STATE.securityDomain;
    });
    var geos = unique(rows.map(function (row) { return row.geo_code; })).filter(Boolean);
    if (!geos.length) geos = ["IT", "EU27_2020"];
    return geos.map(function (geo) {
      return { value: geo, label: securityGeoLabel(geo) };
    }).sort(function (a, b) {
      if (a.value === "IT") return -1;
      if (b.value === "IT") return 1;
      if (a.value === "EU27_2020") return -1;
      if (b.value === "EU27_2020") return 1;
      return sortItalian(a.label, b.label);
    });
  }

  function securityYearOptions(chartId) {
    var rows = securitySpendingRowsForSelection({ ignoreGeo: true });
    if (chartId === "siSecuritySpendingBreakdownChart") {
      rows = securityBreakdownCandidateRows();
    }
    if (chartId === "siSecuritySpendingRankingChart") {
      var yearCounts = {};
      rows.forEach(function (row) {
        if (!isFiniteNumber(row.year) || row.geo_group === "eu_aggregate") return;
        if (!isFiniteNumber(securityTransformedPointValue(row))) return;
        yearCounts[row.year] = (yearCounts[row.year] || 0) + 1;
      });
      var completeEnoughYears = Object.keys(yearCounts)
        .map(numberOrNull)
        .filter(function (year) { return isFiniteNumber(year) && yearCounts[year] >= 5; });
      if (completeEnoughYears.length) {
        return completeEnoughYears.sort(function (a, b) { return b - a; }).map(function (year) {
          return { value: String(year), label: String(year) };
        });
      }
    }
    var years = unique(rows.map(function (row) { return row.year; })).filter(isFiniteNumber).sort(function (a, b) { return b - a; });
  ...
  function renderChartById(id) {
    ...
    return withChartState(id, function () {
      if (isSecuritySpendingChart(id)) {
        return renderSecuritySpendingChartById(id);
      }
  ...
  function isSecuritySpendingChart(id) {
    return id === "siSecuritySpendingTrendChart" || id === "siSecuritySpendingBreakdownChart" || id === "siSecuritySpendingRankingChart";
  }

  function renderSecuritySpendingChartById(id) {
    var needed = neededSecuritySpendingPayloads(id);
    var missing = needed.filter(function (key) { return !STATE.securitySpendingLoaded[key]; });
    if (missing.length) {
      emptyChart(id, ui("Caricamento dati macro sulla sicurezza ...", "Loading macro security data ..."));
      Promise.all(missing.map(loadSecuritySpendingPayload)).then(function () {
        renderChartById(id);
      });
      return;
    }
    populateSecuritySpendingFilters(id);
    updateChartNarrative(id);
    if (id === "siSecuritySpendingTrendChart") return renderSecuritySpendingTrendChart();
    if (id === "siSecuritySpendingBreakdownChart") return renderSecuritySpendingBreakdownChart();
    if (id === "siSecuritySpendingRankingChart") return renderSecuritySpendingRankingChart();
  }

  function neededSecuritySpendingPayloads(id) {
    var keys = [];
    if (STATE.securityDomain === "private_security_market") return ["privateMarket"];
    keys.push("publicTotal");
    if (
      id === "siSecuritySpendingBreakdownChart"
      || STATE.securityClassification !== "GF03"
    ) {
      keys.push("publicDetail");
    }
    return unique(keys);
  }

  function renderSecuritySpendingTrendChart() {
    var rows = securitySpendingRowsForSelection();
    var selectedRows = rows.filter(function (row) { return row.geo_code === STATE.securityGeo; });
    var compareRows = STATE.securityCompareGeo && STATE.securityCompareGeo !== "none"
      ? rows.filter(function (row) { return row.geo_code === STATE.securityCompareGeo; })
      : [];
    var traces = [];
    [
      { geo: STATE.securityGeo, rows: selectedRows },
      { geo: STATE.securityCompareGeo, rows: compareRows }
    ].forEach(function (entry, index) {
      if (!entry.geo || entry.geo === "none" || !entry.rows.length) return;
      var points = transformSecuritySeries(entry.rows);
      traces.push({
        type: "scatter",
        mode: "lines+markers",
        name: securityGeoLabel(entry.geo),
        ...
        marker: { size: 7 },
        hovertemplate: "<b>%{fullData.name}</b><br>" + ui("Anno", "Year") + ": %{x}<br>" + securityMeasureAxisLabel() + ": %{y:,.2f}<extra></extra>"
      });
    });
    if (!traces.length) return emptyChart("siSecuritySpendingTrendChart", ui("Nessuna serie macro per i filtri selezionati.", "No macro series for the selected filters."));
    if (els.siSecuritySpendingTrendTag) els.siSecuritySpendingTrendTag.textContent = securityTagLabel();
    plot("siSecuritySpendingTrendChart", traces, {
      yTitle: securityMeasureAxisLabel(),
      legend: true,
      yearAxis: true,
      axisYears: securityAxisYears(rows)
    });
  }

  function renderSecuritySpendingBreakdownChart() {
    var year = selectedSecurityYear("siSecuritySpendingBreakdownChart");
    var rows = securityBreakdownRows(year).sort(descValue);
    var labels = rows.map(function (row) { return row.label; });
    if (els.siSecuritySpendingBreakdownTag) els.siSecuritySpendingBreakdownTag.textContent = securityDomainLabel() + " - " + year;
    if (!rows.length) return emptyChart("siSecuritySpendingBreakdownChart", ui("Nessun dettaglio macro per i filtri selezionati.", "No macro detail for the selected filters."));
    plot("siSecuritySpendingBreakdownChart", [{
      type: "bar",
      orientation: "h",
      x: rows.map(function (row) { return row.value; }),
      y: labels,
      marker: { color: rows.map(function (_, index) { return COLORS[index % COLORS.length]; }) },
      hovertemplate: "<b>%{y}</b><br>" + securityMeasureAxisLabel() + ": %{x:,.2f}<extra></extra>"
    }], {
      xTitle: securityMeasureAxisLabel(),
      height: barChartHeight(rows.length),
      marginLeft: marginForYLabels(labels, 160, 320),
      yTickVals: labels,
      yTickText: labels
    });
  }

  function renderSecuritySpendingRankingChart() {
    var year = selectedSecurityYear("siSecuritySpendingRankingChart");
    var rows = securityRankingRows(year).sort(function (a, b) { return (b._displayValue || 0) - (a._displayValue || 0); }).slice(0, 24).reverse();
    var labels = rows.map(function (row) { return securityGeoLabel(row.geo_code); });
    if (els.siSecuritySpendingRankingTag) els.siSecuritySpendingRankingTag.textContent = String(year) + " - " + securityMeasureAxisLabel();
    if (!rows.length) return emptyChart("siSecuritySpendingRankingChart", ui("Nessun confronto europeo per i filtri selezionati.", "No European comparison for the selected filters."));
    plot("siSecuritySpendingRankingChart", [{
      type: "bar",
      orientation: "h",
      x: rows.map(function (row) { return row._displayValue; }),
      y: labels,
      marker: { color: rows.map(function (row) { return row.geo_code === "IT" ? COLORS[0] : COLORS[1]; }) },
      hovertemplate: "<b>%{y}</b><br>" + securityMeasureAxisLabel() + ": %{x:,.2f}<extra></extra>"
    }], {
      xTitle: securityMeasureAxisLabel(),
      height: barChartHeight(rows.length),
      marginLeft: marginForYLabels(labels, 150, 300),
      yTickVals: labels,
      yTickText: labels
    });
  }
  ...
  function securitySpendingRows() {
    return STATE.securitySpendingRecords || [];
  }

  function securitySpendingRowsForSelection(options) {
    var opts = options || {};
    var parsed = parseSecurityMeasure();
    return securitySpendingRows().filter(function (row) {
      if (row.macro_domain !== STATE.securityDomain) return false;
      if (!opts.ignoreGeo && row.geo_code !== STATE.securityGeo && row.geo_code !== STATE.securityCompareGeo) return false;
      if (STATE.securityDomain === "public_security_spending") {
        return row.classification_code === STATE.securityClassification && row.unit_code === parsed.unitCode;
      }
      return row.classification_code === STATE.securityClassification
        && row.measure_code === parsed.measureCode
        && row.unit_code === parsed.unitCode;
    });
  }

  function parseSecurityMeasure() {
    if (STATE.securityDomain === "private_security_market") {
      var parts = String(STATE.securityMeasure || "").split("||");
      return { measureCode: parts[0] || "NETTUR_MEUR", unitCode: parts[1] || "MIO_EUR" };
    }
    return {
      measureCode: STATE.securityMeasure || SECURITY_PUBLIC_DEFAULT_MEASURE,
      unitCode: STATE.securityMeasure || SECURITY_PUBLIC_DEFAULT_MEASURE
    };
  }

  function securityPrivateMeasureKey(row) {
    if (!row || !row.measure_code || !row.unit_code) return "";
    return row.measure_code + "||" + row.unit_code;
  }

  function securityPrivateMeasureLabel(row) {
    return tr(row.measure_name || labelize(row.measure_code || "")) + " - " + securityUnitLabel(row.unit_code, row.unit_name);
  }

  function transformSecuritySeries(rows) {
    var points = rows.slice().sort(function (a, b) { return a.year - b.year; }).map(function (row) {
      return { year: row.year, value: securityDisplayValue(row), raw: row };
    });
    if (STATE.securityTransform === "index") {
      var base = points.find(function (point) { return isFiniteNumber(point.value) && point.value !== 0; });
      return points.map(function (point) {
        return { year: point.year, value: base ? point.value / base.value * 100 : null, raw: point.raw };
      });
    }
    if (STATE.securityTransform === "yoy") {
      return points.map(function (point, index) {
        var previous = index > 0 ? points[index - 1] : null;
        return { year: point.year, value: previous ? percentChange(point.value, previous.value) : null, raw: point.raw };
      });
    }
    return points;
  }

  function securityDisplayValue(row) {
    if (!row || !isFiniteNumber(row.value)) return null;
    if (row.unit_code === "MIO_EUR") return Number(row.value) / 1000;
    return Number(row.value);
  }

  function selectedSecurityYear(chartId) {
    var options = securityYearOptions(chartId);
    if (!STATE.securityYear || !options.some(function (option) { return Number(option.value) === Number(STATE.securityYear); })) {
      STATE.securityYear = options.length ? numberOrNull(options[0].value) : null;
    }
    return STATE.securityYear;
  }

  function securityBreakdownRows(year) {
    return securityBreakdownCandidateRows().filter(function (row) {
      return row.year === year;
    }).map(function (row) {
      return { label: securityClassificationLabel(row.classification_code), value: securityTransformedPointValue(row), raw: row };
    }).filter(function (row) { return isFiniteNumber(row.value); });
  }

  function securityBreakdownCandidateRows() {
    var parsed = parseSecurityMeasure();
    if (STATE.securityDomain === "public_security_spending") {
      return securitySpendingRows().filter(function (row) {
        return row.macro_domain === "public_security_spending"
          && row.geo_code === STATE.securityGeo
          && row.classification_code !== "GF03"
          && row.unit_code === parsed.unitCode;
      });
    }
    return securitySpendingRows().filter(function (row) {
      return row.macro_domain === "private_security_market"
        && row.geo_code === STATE.securityGeo
        && row.classification_code !== "N80"
        && row.measure_code === parsed.measureCode
        && row.unit_code === parsed.unitCode;
    });
  }

  function securityRankingRows(year) {
    return securitySpendingRowsForSelection({ ignoreGeo: true }).filter(function (row) {
      return row.year === year && row.geo_group !== "eu_aggregate";
    }).map(function (row) {
      row._displayValue = securityTransformedPointValue(row);
      return row;
    }).filter(function (row) { return isFiniteNumber(row._displayValue); });
  }

  function securityTransformedPointValue(row) {
    if (STATE.securityTransform === "value") return securityDisplayValue(row);
    var allRows = securitySpendingRows().filter(function (candidate) {
      if (candidate.macro_domain !== row.macro_domain) return false;
      if (candidate.geo_code !== row.geo_code) return false;
      if (candidate.classification_code !== row.classification_code) return false;
      if (candidate.unit_code !== row.unit_code) return false;
      if (row.macro_domain === "private_security_market" && candidate.measure_code !== row.measure_code) return false;
      return true;
    });
    var transformed = transformSecuritySeries(allRows).find(function (point) { return point.year === row.year; });
    return transformed ? transformed.value : null;
  }

  function securityAxisYears(rows) {
    return unique((rows || securitySpendingRowsForSelection()).map(function (row) { return row.year; })).filter(isFiniteNumber).sort(function (a, b) { return a - b; });
  }

  function securityGeoLabel(code) {
    var found = securitySpendingRows().find(function (row) { return row.geo_code === code && row.geo_name; });
    if (found) return tr(found.geo_name);
    var fallback = {
      EU27_2020: ui("Unione europea (27)", "European Union (27)"),
      IT: ui("Italia", "Italy"),
      DE: ui("Germania", "Germany"),
      FR: ui("Francia", "France"),
      ES: ui("Spagna", "Spain")
    };
    return fallback[code] || code;
  }

  function securityClassificationLabel(code) {
    var options = SECURITY_PUBLIC_CLASSIFICATIONS.concat(SECURITY_PRIVATE_CLASSIFICATIONS);
    var found = options.find(function (option) { return option.value === code; });
    return found ? tr(found.label) : code;
  }

  function securityUnitLabel(unitCode, fallback) {
    if (unitCode === "MIO_EUR") return ui("milioni di euro", "million euro");
    if (unitCode === "EUR_HAB_COMPUTED") return ui("euro per abitante", "euro per inhabitant");
    if (unitCode === "PC_GDP") return ui("% del PIL", "% of GDP");
    if (unitCode === "PC_TOT") return ui("% della spesa pubblica", "% of total public spending");
    if (unitCode === "NR") return ui("numero", "number");
    return tr(fallback || unitCode || "");
  }

  function securityDomainLabel() {
    var found = SECURITY_DOMAIN_OPTIONS.find(function (option) { return option.value === STATE.securityDomain; });
    return found ? tr(found.label) : tr(STATE.securityDomain);
  }

  function securityMeasureAxisLabel() {
    if (STATE.securityTransform === "index") return ui("Indice primo anno = 100", "Index first year = 100");
    if (STATE.securityTransform === "yoy") return ui("Variazione % annua", "Annual % change");
    var parsed = parseSecurityMeasure();
    if (parsed.unitCode === "MIO_EUR") return ui("Miliardi di euro", "Billion euro");
    if (parsed.unitCode === "EUR_HAB_COMPUTED") return ui("Euro per abitante", "Euro per inhabitant");
    if (parsed.unitCode === "PC_GDP") return ui("% del PIL", "% of GDP");
    if (parsed.unitCode === "PC_TOT") return ui("% della spesa pubblica", "% of total public spending");
    if (parsed.unitCode === "NR") return ui("Numero", "Number");
    return tr(parsed.unitCode);
  }

  function securityTagLabel() {
    return securityDomainLabel() + " - " + securityClassificationLabel(STATE.securityClassification);
  }
  ...
  function plot(id, traces, options) {
    ...
    if (options && options.yearAxis) {
      var sourceYears = options.axisYears && options.axisYears.length ? options.axisYears : years();
      var axisYears = sourceYears.map(yearLabel);
  ...
  function updateChartNarrative(id) {
    ...
    if (credit) {
      credit.innerHTML = chartCreditHtml(narrative.note, narrative.source);
    }
  }

  function chartCreditHtml(note, source) {
    var sourceText = source || ui("Fonte: ISTAT. Elaborazione di Nazareno Lecis.", "Source: ISTAT. Processing by Nazareno Lecis.");
    return '<span class="si-chart-source">' + escapeHtml(sourceText) + '</span><br><span class="si-chart-note"><strong>' + escapeHtml(ui("Nota:", "Note:")) + "</strong> " + escapeHtml(note || "") + "</span>";
  }

  function normalizeStaticChartCredits() {
    document.querySelectorAll(".si-chart-credit").forEach(function (credit) {
      if (credit.querySelector(".si-chart-source")) return;
      var text = credit.textContent.replace(/\s+/g, " ").trim();
      var split = splitCreditText(text);
      if (!split) return;
      credit.innerHTML = chartCreditHtml(split.note, split.source);
    });
  }

  function splitCreditText(text) {
    var sourceMatch = text.match(/\b(Fonte:|Source:)\s+/);
    if (!sourceMatch || sourceMatch.index <= 0) return null;
    var note = text.slice(0, sourceMatch.index).replace(/^Nota(?:\s+[^:]+)?:\s*/i, "").trim();
    var source = text.slice(sourceMatch.index).trim();
  ...
  function chartNarrative(id) {
    ...
    if (id === "siCompositionChart") {
      return {
        ...
        note: ui("Mostra il peso di categorie o singoli reati nel perimetro selezionato; la quota percentuale è calcolata sul totale visualizzato.", "Shows the weight of categories or individual offences within the selected scope; the percentage share is calculated on the displayed total.") + reportedAccessNote()
      };
    }
    if (id === "siContributionChart") {
      return {
        title: ui("Variazioni ", "Changes ") + normalizedComparisonYear() + "-" + STATE.year + ": " + metric + " - " + scope,
        note: ui("Include i reati che superano la soglia scelta e rispetta il filtro direzione; usa la variazione percentuale rispetto all'anno di confronto.", "Includes offences above the selected threshold and respects the direction filter; it uses percentage change against the comparison year.") + reportedAccessNote()
      };
    }
    ...
    if (id === "siScatterChart") {
      return {
        ...
        note: ui("L'asse orizzontale mostra ", "The horizontal axis shows ") + measure.toLowerCase() + ui("; l'asse verticale mostra la variazione percentuale annua.", "; the vertical axis shows annual percentage change.") + reportedAccessNote()
      };
    }
    ...
    if (id === "siCounterChart") {
      return {
        ...
        note: ui("Mostra territori e reati che si muovono in direzione opposta rispetto all'Italia, secondo il filtro Scostamenti selezionato.", "Shows territories and offences moving in the opposite direction from Italy, according to the selected Divergences filter.") + reportedAccessNote()
      };
    }
    ...
    if (id === "siSecuritySpendingTrendChart") {
      return {
        title: ui("Serie storica macro: ", "Macro time series: ") + securityTagLabel() + " - " + securityGeoLabel(STATE.securityGeo) + " - " + securityMeasureAxisLabel(),
        note: securitySpendingNote() + securitySpendingComparisonNote(),
        source: securitySpendingSource()
      };
    }
    if (id === "siSecuritySpendingBreakdownChart") {
      return {
        title: ui("Scomposizione macro nel ", "Macro breakdown in ") + selectedSecurityYear("siSecuritySpendingBreakdownChart") + " - " + securityDomainLabel() + " - " + securityGeoLabel(STATE.securityGeo),
        note: securitySpendingNote() + ui(" Il grafico mostra le sottovoci disponibili per il dominio scelto: COFOG GF03 per la spesa pubblica o sottosettori NACE N80 per la sicurezza privata.", " The chart shows available sub-items for the selected domain: COFOG GF03 for public spending or NACE N80 sub-sectors for private security."),
        source: securitySpendingSource()
      };
    }
    if (id === "siSecuritySpendingRankingChart") {
      return {
        title: ui("Confronto europeo nel ", "European comparison in ") + selectedSecurityYear("siSecuritySpendingRankingChart") + " - " + securityTagLabel() + " - " + securityMeasureAxisLabel(),
        note: securitySpendingNote() + ui(" Il ranking confronta paesi europei quando la misura e l'anno sono disponibili.", " The ranking compares European countries when the selected measure and year are available."),
        source: securitySpendingSource()
      };
    }
  ...
  function measureNote() {
    var note = ui("Valori assoluti: conteggio dei delitti denunciati nel perimetro selezionato.", "Absolute values: count of reported crimes within the selected scope.");
    if (STATE.measure === "rate") note = ui("Tasso per 100.000 abitanti: valore rapportato alla popolazione residente del territorio e anno selezionati.", "Rate per 100,000 inhabitants: value divided by resident population for the selected territory and year.");
    if (STATE.measure === "rate_1000") note = ui("Tasso per 1.000 abitanti: valore rapportato alla popolazione residente del territorio e anno selezionati.", "Rate per 1,000 inhabitants: value divided by resident population for the selected territory and year.");
    if (STATE.measure === "index") note = ui("Indice con primo anno uguale a 100: evidenzia la dinamica relativa della serie, non il volume assoluto.", "Index with first year equal to 100: highlights the relative dynamics of the series, not the absolute volume.");
    if (STATE.measure === "moving_average") note = ui("Media mobile triennale: attenua oscillazioni annuali e va letta come andamento smussato.", "Three-year moving average: smooths yearly fluctuations and should be read as a smoothed trend.");
    if (STATE.measure === "yoy") note = ui("Variazione percentuale annua: confronto con l'anno precedente nello stesso perimetro.", "Annual percentage change: comparison with the previous year in the same scope.");
    return note + reportedAccessNote();
  }

  function reportedAccessNote() {
    return ui(" I valori possono cambiare anche per propensione o possibilità concreta di denunciare: barriere burocratiche, linguistiche o di accesso ai servizi possono ridurre le denunce; campagne informative, canali più semplici o cambiamenti normativi possono aumentarle senza indicare necessariamente più reati commessi.", " Values can also change because of people's propensity or practical ability to report: bureaucratic, language or service-access barriers can reduce reports; information campaigns, simpler channels or legal changes can increase reports without necessarily indicating more offences committed.");
  }
  ...
  function surveyNarrativeNote(topic) {
    ...
    var sample = surveySampleSizeNote(rows);
    if (topic === "reporting_propensity") {
      return ui("Propensione alla denuncia da indagine campionaria: ", "Reporting propensity from sample survey: ") + unit + ui(". I periodi sono edizioni dell'indagine, non anni di una serie annuale; il grafico non somma periodi diversi. ", ". Periods are survey editions, not annual time-series years; the chart does not add different periods together. ") + sample + universe + ui(" Nel payload attuale la propensione alla denuncia è disponibile per reato, ripartizione geografica e motivi della denuncia o della non denuncia, ma non per sesso, età, cittadinanza o status migratorio: per quella domanda servono microdati o tavole dedicate.", " In the current payload, reporting propensity is available by offence, geographic area and reasons for reporting or not reporting, but not by sex, age, citizenship or migration status: that question requires microdata or dedicated tables.");
    }
    if (topic === "victimization") {
      return ui("Vittimizzazione dichiarata da indagine campionaria: ", "Reported victimization from sample survey: ") + unit + ui(". Include anche fenomeni non denunciati e non va sommata ai delitti registrati. ", ". It also includes unreported events and should not be added to registered crimes. ") + sample + universe;
    }
    return ui("Percezione della sicurezza da indagine campionaria: ", "Perceived safety from sample survey: ") + unit + ui(". Misura opinioni, rischio percepito e comportamenti dichiarati, non denunce registrate. ", ". It measures opinions, perceived risk and stated behaviours, not registered reports. ") + sample + universe;
  }

  function securitySpendingNote() {
    var base = "";
    if (STATE.securityDomain === "public_security_spending") {
      base = ui(
        "Spesa delle amministrazioni pubbliche per ordine pubblico e sicurezza secondo COFOG GF03; la difesa nazionale COFOG GF02 non è inclusa.",
        "General-government spending on public order and safety under COFOG GF03; national defence COFOG GF02 is not included."
      );
    } else {
      base = ui(
        "La sicurezza privata è una proxy di mercato basata sulle statistiche strutturali delle imprese NACE N80: misura fatturato, valore aggiunto, imprese o occupazione del settore, non tutta la spesa privata effettiva.",
        "Private security is a market proxy based on structural business statistics for NACE N80: it measures turnover, value added, enterprises or employment in the sector, not all actual private spending."
      );
    }
    if (STATE.securityTransform === "index") {
      base += " " + ui("L'indice pone a 100 il primo anno disponibile della serie selezionata.", "The index sets the first available year in the selected series equal to 100.");
    }
    if (STATE.securityTransform === "yoy") {
      base += " " + ui("La variazione percentuale annua confronta ogni anno con l'anno precedente disponibile nella stessa serie.", "The annual percentage change compares each year with the previous available year in the same series.");
    }
    if (STATE.securityDomain === "public_security_spending" && parseSecurityMeasure().unitCode === "PC_GDP") {
      base += " " + ui("Il dato predefinito è espresso in percentuale del PIL, non in valori assoluti.", "The default value is expressed as a percentage of GDP, not as absolute values.");
    }
    return base;
  }

  function securitySpendingComparisonNote() {
    if (!STATE.securityCompareGeo || STATE.securityCompareGeo === "none") {
      return ui(" Con Nessun confronto il grafico mostra solo il paese selezionato.", " With No comparison the chart shows only the selected country.");
    }
    return ui(" Il filtro Confronta con aggiunge una seconda serie macro.", " The Compare with filter adds a second macro series.");
  }

  function securitySpendingSource() {
    if (STATE.securityDomain === "private_security_market") {
      return ui(
        "Fonte: Eurostat sbs_ovw_act, NACE N80 investigazione e servizi di sicurezza. Elaborazione di Nazareno Lecis.",
        "Source: Eurostat sbs_ovw_act, NACE N80 security and investigation activities. Processing by Nazareno Lecis."
      );
    }
    return ui(
      "Fonte: Eurostat gov_10a_exp, spesa pubblica COFOG GF03 ordine pubblico e sicurezza; popolazione Eurostat demo_pjan usata solo per i valori per abitante. Elaborazione di Nazareno Lecis.",
      "Source: Eurostat gov_10a_exp, COFOG GF03 public order and safety; Eurostat demo_pjan population is used only for per-inhabitant values. Processing by Nazareno Lecis."
    );
  }

  function surveySampleSizeNote(rows) {
    var periods = unique((rows || []).map(function (row) { return row.period; })).filter(Boolean);
    var pieces = [];
    if (periods.indexOf("2022-2023") >= 0) {
      pieces.push(ui("Campione 2022-2023: 29.317 individui residenti in Italia di 14 anni e più, intervistati tra novembre 2022 e luglio 2023 con tecnica mista telefonica e faccia a faccia; tasso di risposta complessivo 76,8%.", "2022-2023 sample: 29,317 Italy-resident people aged 14+, interviewed between November 2022 and July 2023 by mixed telephone and face-to-face mode; overall response rate 76.8%."));
    }
    if (periods.indexOf("2015-2016") >= 0) {
      pieces.push(ui("Campione 2015-2016: circa 50.350 interviste individuali, di cui circa 43.000 telefoniche e 7.350 faccia a faccia.", "2015-2016 sample: about 50,350 individual interviews, around 43,000 by telephone and 7,350 face to face."));
    }
    if (periods.some(function (period) { return ["2022-2023", "2015-2016"].indexOf(period) < 0; })) {
      pieces.push(ui("Per le altre edizioni storiche la numerosità campionaria varia: leggere la nota metodologica ISTAT dell'edizione di riferimento.", "For other historical editions the sample size varies: read the ISTAT methodology note for the relevant edition."));
    }
