(function () {
  ...
  function readUnder500Rows(node) {
    ...
    return bar.x.map(function (label, index) {
      return {
        label: String(label || ""),
        value: toNumber(bar.y[index])
      };
    }).filter(function (row) {
      return row.label && row.value !== null;
    });
  }
  ...
  function tuneUnder500() {
    ...
    if (freshRows && freshRows.length) {
      node.__bpUnder500Rows = freshRows;
      node.__bpUnder500Tuned = false;
    }

    if (node.__bpUnder500Tuned) return;
    if (!node.__bpUnder500Rows || !node.__bpUnder500Rows.length) return;

    var rows = node.__bpUnder500Rows.slice()
    ...
    window.Plotly.react(node, [{
      ...
      y: rows.map(function (row) { return shortLabel(row.label); }),
      customdata: rows.map(function (row) { return row.label; }),
      marker: { color: cssVar("--orange", "#ff5a1f") },
      text: rows.map(function (row) { return formatMld(row.value); }),
      textposition: "auto",
      hovertemplate: "%{customdata}<br>%{x:.3f} mld<extra></extra>"
    }], {
      height: 430,
      margin: { t: 6, r: 8, b: 44, l: 170 },
      showlegend: false,
      paper_bgcolor: "rgba(0,0,0,0)",
      plot_bgcolor: "rgba(0,0,0,0)",
      font: {
        color: cssVar("--text", "#f5f2ed"),
      ...
      xaxis: {
        title: "Miliardi",
        fixedrange: true,
        gridcolor: cssVar("--line", "#303030"),
        zerolinecolor: cssVar("--line", "#303030"),
        tickfont: { color: cssVar("--muted", "#b9b2aa") },
        titlefont: { color: cssVar("--muted", "#b9b2aa") },
        automargin: false
      },
      yaxis: {
        title: "",
  ...
  function schedule() {
    tuneAll();
    [250, 800, 1500, 2500].forEach(function (delay) {
      window.setTimeout(tuneAll, delay);
    });
  }
  ...
  function observeCharts() {
    ...
    OBSERVED_CHART_IDS.forEach(function (id) {
      ...
      var observer = new MutationObserver(function () {
        window.clearTimeout(node.__bpChartFixTimer);
        node.__bpChartFixTimer = window.setTimeout(tuneAll, 80);
      });
  ...
  window.addEventListener("resize", function () {
    var under500 = document.getElementById("bpUnder500Chart");
    if (under500) under500.__bpUnder500Tuned = false;
    schedule();
  });
})();
