(function () {
  ...
  function translateDashboardStatic() {
    ...
    if (meta) {
      meta.setAttribute("content", ui(
        "Dashboard sulla sicurezza in Italia: criminalita registrata, territori, reati, autori, vittime, cittadinanza, trend, fonti e metodologia.",
  ...
  function renderPeopleNotice() {
    ...
    if (!STATE.peopleLoaded && !people.length) {
      els.siPeopleNotice.innerHTML = "<strong>" + escapeHtml(ui("Copertura persone:", "People coverage:")) + "</strong> " + escapeHtml(ui("caricamento in corso. I grafici principali sui reati sono gia disponibili.", "loading. The main crime charts are already available."));
      return;
    }
    if (!people.length) {
  ...
  function refreshDependentFilters() {
    ...
    populateControlGroup("region", [{ value: "all", label: ui("Tutte", "All") }].concat(regions.map(optionFromValue)), STATE.region);
    ...
    populateControlGroup("territory", [{ value: "all", label: ui("Tutti", "All") }].concat(territoryOptions), STATE.territory);
    if (STATE.compareTerritory !== "all" && !territoryOptions.some(function (option) { return option.value === STATE.compareTerritory; })) {
      STATE.compareTerritory = "all";
    }
    populateControlGroup("compareTerritory", [{ value: "all", label: ui("Nessun confronto", "No comparison") }].concat(territoryOptions), STATE.compareTerritory);
  ...
  function markDeferredChart(id) {
    ...
    if (chart && !chart.dataset.deferred) {
      ...
      chart.innerHTML = '<div class="si-chart-empty">' + escapeHtml(ui("Il grafico verra caricato quando arrivi a questa sezione.", "This chart will load when you reach this section.")) + "</div>";
    }
  }
  ...
  function renderDemographyChart() {
    ...
    els.siDemographyTag.textContent = rows.length ? roleLabel(STATE.role) + " - " + ui("sesso ed eta", "sex and age") + " - " + peopleMeasureLabel() : ui("nessun dato", "no data");
    if (!rows.length) return emptyChart("siDemographyChart", ui("Sesso ed eta non risultano nel perimetro selezionato.", "Sex and age are not available in the selected scope."));
    var grouped = peopleMetricGroups(rows, function (row) {
  ...
  function renderSurveyChart(chartId, tagId, rows, title, stateKey) {
    ...
    if (tag) tag.textContent = selectedIndicator === "all" ? "ISTAT 2015-2016 vs 2022-2023" : tr(selectedIndicator);
    ...
    if (!rows.length) return emptyChart(chartId, ui("Dati campionari non disponibili.", "Sample-survey data are not available."));
    plot(chartId, periods.map(function (period, index) {
      ...
      xTitle: ui("% popolazione/famiglie di riferimento", "% reference population/households"),
      yTitle: "",
      legend: true,
      marginLeft: marginForYLabels(indicators.map(tr), 230, 320),
      barmode: "group",
      height: Math.max(250, indicators.length * 48 + 110),
      yTickVals: indicators.map(tr),
      yTickText: indicators.map(tr)
    });
  }
  ...
  function peopleRateUnavailableMessage() {
    if (STATE.peopleMeasure === "rate" || STATE.peopleMeasure === "rate_1000") {
      return ui(
        "Non riesco a calcolare il tasso per i filtri selezionati perche manca la popolazione del territorio e anno corrispondenti. Usa valori assoluti o quota %.",
  ...
  function measureUnavailableMessage(scope) {
    if (STATE.measure === "rate" || STATE.measure === "rate_1000") {
      return ui("Non riesco a calcolare il tasso per ", "The selected rate cannot be calculated for ") + scope + ui(" perche manca la popolazione del territorio e anno corrispondenti.", " because the matching population for territory and year is missing.");
    }
  ...
  function selectedSeriesEntities(base) {
    var entities = [{ level: "national", code: "IT", label: levelLabel("national") }];
  ...
  function homicideCodeKind(code) {
    ...
    if (code === "ROADHOM") return ui("voce specifica: puo non essere sommabile agli aggregati", "specific item: it may not be additive with aggregates");
  ...
  function homicideAgeOptions() {
    ...
    return [{ value: "all", label: ui("Tutte le eta", "All ages") }].concat(values.map(function (value) {
  ...
  function chartNarrative(id) {
    ...
    if (id === "siTrendChart") {
      return {
        ...
        note: measureNote() + ui(" L'Italia resta il benchmark quando selezioni un territorio o un confronto.", " Italy remains the benchmark when you select a territory or comparison.")
      };
    }
    if (id === "siCompositionChart") {
      return {
        title: ui("Composizione dei reati nel ", "Offence composition in ") + STATE.year + " - " + scope,
        note: ui("Mostra il peso di categorie o singoli reati nel perimetro selezionato; la quota percentuale e calcolata sul totale visualizzato.", "Shows the weight of categories or individual offences within the selected scope; the percentage share is calculated on the displayed total.")
      };
    }
    ...
    if (id === "siTreemapChart") {
      return {
        ...
        note: measureNote() + ui(" Mostra i territori con valore piu alto nel perimetro geografico selezionato.", " Shows the territories with the highest value within the selected geographic scope.")
      };
    }
    ...
    if (id === "siViolentChart") {
      return {
        ...
        note: measureNote() + ui(" La separazione e operativa sui codici reato e non sostituisce una classificazione giuridica esaustiva.", " The split is operational on offence codes and does not replace an exhaustive legal classification.")
      };
    }
    ...
    if (id === "siCrimeChart") {
      return {
        ...
        note: measureNote() + ui(" La serie usa il reato specifico selezionato; se il filtro e su tutti, viene mostrata la voce piu rilevante nel perimetro.", " The series uses the selected individual offence; if the filter is set to all, it shows the most relevant item in the selected scope.")
      };
    }
    if (id === "siHomicideTrendChart") {
      return {
        title: ui("Omicidi: serie storica ", "Homicides: time series ") + homicideTypeLabel() + " - " + scope + " - " + measure,
        note: measureNote() + ui(" La serie confronta l'Italia con il territorio selezionato o con il secondo territorio scelto.", " The series compares Italy with the selected territory or the second chosen territory.")
      };
    }
    ...
    if (id === "siHomicidePeopleChart") {
      return {
        ...
        note: peopleMeasureNote() + ui(" Il payload autori/vittime caricato ora e nazionale.", " The currently loaded offender/victim payload is national.")
      };
    }
    if (id === "siPeopleChart") {
      return {
        title: roleLabel(STATE.role) + ui(" per cittadinanza e reato - ", " by citizenship and offence - ") + peopleMeasureLabel(),
        note: peopleMeasureNote() + ui(" Autori e vittime restano separati dal filtro Ruolo; il payload persone caricato ora e nazionale.", " Offenders and victims remain separated by the Role filter; the currently loaded people payload is national.")
      };
    }
    if (id === "siDemographyChart") {
      return {
        title: ui("Identikit ", "Profile ") + roleLabel(STATE.role).toLowerCase() + ui(": sesso ed eta - ", ": sex and age - ") + peopleMeasureLabel(),
        note: peopleMeasureNote() + ui(" Il dettaglio per sesso ed eta dipende dalle dimensioni disponibili nel payload persone.", " Sex and age detail depends on the dimensions available in the people payload.")
      };
    }
    ...
    if (id === "siPerceptionChart") {
      return {
        title: ui("Percezione della sicurezza: indagine campionaria", "Perceived safety: sample survey"),
        note: ui("Quote di persone o famiglie nelle indagini campionarie; misura fenomeni diversi dalle denunce registrate.", "Shares of people or households in sample surveys; this measures phenomena different from registered reports.")
      };
    }
    if (id === "siVictimizationChart") {
      return {
        title: ui("Vittimizzazione dichiarata: indagine campionaria", "Reported victimization: sample survey"),
        note: ui("Quote di persone o famiglie che dichiarano di aver subito reati; include anche fenomeni non denunciati.", "Shares of people or households reporting that they experienced offences; this also includes unreported phenomena.")
      };
    }
  ...
  function homicidePeopleBreakdownLabel() {
    ...
    return ui("sesso ed eta", "sex and age");
  }
